...
public class ChatHub : Hub
{
    ...
    public async Task SendMessage(Guid sessionId, string message)
    {
        ...
        try
        {
            ...
            if (session == null)
            {
                await Clients.Caller.SendAsync("ReceiveMessage", "Error: Sesi√≥n no encontrada o no autorizada.");
                return;
            }
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (session == null)
                {
                    errorMessage = "Error: Sesi√≥n no encontrada o no autorizada.";
                }
                else 
                {
                    _sessionService.EnqueueUserMessage(session, message);
        ...
        catch (Exception ex)
        {
            ...
            yield return "\n\n[Error: La generaciÛn de texto se interrumpiÛ.]";
        }
    ...
    private static ChatHistory BuildChatHistory(List<ChatMessage> messages)
    {
        var history = new ChatHistory(
            "Eres un asistente virtual del ayuntamiento. Responde de forma clara y concisa en espa√±ol.");
